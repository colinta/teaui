...
export class TestTerminal implements SGRTerminal {
   ...
   * An SGR reset clears visual attributes, but does not end an OSC 8 link.
   *
  ...
  #parseAccumulatedSGR(sgr: string): Style {
    const sequences = unicode.printableChars(sgr)
    let style = this.#currentStyle
    for (const seq of sequences) {
      if (seq === '\u001b[0m' || seq === '\u001b[m') {
        style = new Style({link: style.link, linkParams: style.linkParams})
      } else {
